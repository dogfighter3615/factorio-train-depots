...
function check_active_station_count(v)
    local activecount = 0
    if v.go_to_depot then
        if v.train.schedule then
            
            for _,station in pairs(v.train.schedule.records) do
                if station.station ~= v.selected_depot then
                    if global.trainstop_table[station.station] then
        ...
        if activecount > 1 then
            for key,k in pairs(v.train.schedule.records) do
                if k.station == v.selected_depot then
                    schedule = remove_stop_from_schedule(v.train.schedule,key,v.selected_depot)
                    replace_train_schedule(get_train_by_id(key).train,schedule)
                    v.go_to_depot = false
                end
            end
        end
...
function check_active_station_count_all()
    for _,v in pairs(global.train_table) do
        check_active_station_count(v)
    end
end
...
script.on_event(defines.events.on_train_schedule_changed, function (event)
    ...
    check_active_station_count(global.train_table[event.train.id])
